...
\begin{definition}
    Fix $n \in \NN$, and let $\mathbb{k}$ be a commutative ring.
    ...
    We will denote the set of all such polynomials $\pring{\KK}{x_1,\ldots,x_n}^{S_n}$.
\end{definition}
...
Now, we give the canonized definition for ``the ring of symmetric functions''.
...
The reason for this mouthful is that we want a space into which all of the facts we care about in the theory of symmetric \textit{polynomials} lift up.

Unfortunately, this means we \textit{have} to do some paperwork with regards to constructing this ring.
\begin{definition}
    Fix a ground ring $\KK$.
    The ring $\fps{\KK}{\bfx_\infty}$ of \textit{formal power series in countably many variables} $\bfx_\infty \coloneq (x_1, x_2, \ldots)$ is defined to be the set of all formal linear combinations 
    \[
...
And, a composition is a partition in which we care about the particular order the positive integers are summed.

We give formalizations of these ideas that are convenient in developing the theory of symmetric functions.

\begin{definition}
    A \textit{weak composition}, which we will often refer to simply as a \textit{composition}, $\alpha$ of $n \in \NN$ is a sequence of nonnegative integers 
    \[
        (\alpha_1,\alpha_2,\ldots)
    \]
    such that 
    \[
        ...
        n.
    \]
    The nonzero entries of $\alpha$ are called the \textit{parts} of $\alpha$.
\end{definition}
...
\begin{definition}
    A \textit{partition}, also sometimes called an \textit{integer partition}, $\lambda$ of $n \in \NN$ is a sequence of nonnegative integers 
    \[
    ...
    We say that $n$ is the \textit{size} of $\lambda$.
    The parts of $\lambda$ are the nonzero entries of $\lambda$.
    The \textit{length} of a partition $\lambda$, $\len \lambda$ or $\ell(\lambda)$, is the number of nonzero parts of $\lambda$.
\end{definition}
...
\begin{definition}
    ...
    The set of all partitions $\lambda$ such that $\lambda \vdash n$ will be denoted $\Par_n$.
    The set of all partitions of length $m$ will be denoted $\Par_{\ell=m}$.
    Similarly, the set of all partitions of size at most $n$ and length at most $m$ will be denoted $\Par_{\sum\leq n}$ and $\Par_{\ell \leq m}$ respectively.

    We denote the set of all compositions by $\Com$.
    The same notation for picking out subsets of $\Par$ applies to picking out subsets of $\Com$.
...
\begin{definition}
    Let $\lambda \vdash n$. The \textit{diagram} of $\lambda$, which we will denote $\boxplus \lambda$, is the set
    \[
...
\begin{definition}
    Let $\lambda \vdash n$.
    The \textit{Young diagram} of $\lambda$ is obtained by drawing a box at location $(i,j)$ for each $(i,j)$ in $\boxplus\lambda$.
...
\begin{theorem}
    Standard Young tableau are in bijection with saturated chains in Young's lattice that begin at $\varnothing$.
\end{theorem}
...
The next two orders are not so sensitive to a partition's size, because they are defined not from viewing partitions as subsets of $\NN^2$, but viewing them as sequences.
...
\begin{definition}[Lexicographic order]
    ...
    This is the \textit{lexicographic order} on $\Par$.
\end{definition}

\begin{remark}[Lexicographic order is a total order]
    For \textit{any} partitions $\lambda, \mu \in \Par$, exactly one of the statements
    \[
        \lambda \prec \mu, \qquad \lambda = \mu, \qquad \lambda \succ \mu,
    \]
    holds.
\end{remark}

\begin{theorem}[Lexicographic order is a linear extension of dominance]
...
\begin{proposition}
    Fix $m \in \NN$.
    Partition transposition is a bijection between partitions of length $m$ and partitions whose largest part is $m$.
\end{proposition}
...
The above definition involves permuting around the exponent vector $\alpha$. I personally find it easier to think of the monomial symmetric functions as \textit{permuting the subscripts}. 
...
\begin{definition}
    ...
    \[
        e_n \coloneq \sum_{i_1<i_2<\ldots<i_n} x_{i_1}x_{i_2}\cdots x_{i_n}.
    \]
    And if we let $\lambda \in \Par$, the elementary symmetric function $e_\lambda$ is defined to be
    \[
        e_\lambda \coloneq e_{\lambda_1}e_{\lambda_2}\cdots.
    \]
\end{definition}
...
\begin{definition}
    ...
    \[
        p_n \coloneq \sum_{i\in\PP} x_i^n
    \]
    And if we let $\lambda \in \Par$, the elementary symmetric function $p_\lambda$ is defined to be
    \[
        p_\lambda \coloneq p_{\lambda_1}p_{\lambda_2}\cdots.
    \]
\end{definition}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        M_{\alpha\beta}\ulx^\alpha \uly^\beta \\
        &=
        ...
        \left[
            \sum_{\mu\in\Par}
            M_{\lambda\mu} m_\mu(\uly)
        \right] \\
        &=
        \sum_{\lambda \in \Par}
        m_\lambda(\ulx) e_\lambda(\uly).
    \end{align*}
\end{proof}
...
\begin{theorem}\label{CauchyIdentityForMH}
    We have that
    \[
        \prod_{i,j \geq 1} (1-x_iy_j)^{-1} = \sum_{\lambda\in\Par} m_\lambda(\ulx) h_\lambda(\uly).
    \]
\end{theorem}
...
\begin{proof}
    ...
    \begin{align*}
        \prod_{i,j\geq1}(1-x_iy_j)^{-1} &= \sum_{A \in \Mat_\infty(\NN)}x^{\row A}y^{\col A} \\
                                   &= \sum_{\lambda,\mu\in\Par} N_{\lambda\mu} (m_\lambda(X) m_\mu(Y)) \\
                                   &= \sum_{\lambda\in\Par} m_\lambda(X) \left[\sum_\mu N_{\lambda\mu} m_\mu(Y)\right]\\
                                   &= \sum_{\lambda\in\Par} m_\lambda(X) h_\lambda(Y).
    \end{align*}
\end{proof}
...
\begin{theorem}\label{CauchyIdentityForPP}
    ...
    \[
        ...
        =
        \sum_{\lambda\in\Par}
        z_\lambda^{-1} p_\lambda(\ulx) p_\lambda(\uly).
    \]
    Additionally,
    \[
        \prod_{i,j \geq 1}
        ...
        =
        \sum_{\lambda\in\Par}
        z_\lambda^{-1}\varepsilon_{\lambda} p_\lambda(\ulx) p_\lambda(\uly).
    \]
\end{theorem}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &=
        \sum_{n \geq 1}
        \frac{1}{n} p_n(\ulx) p_n(\uly).
    \end{align*}
    Hence we have that
    \[
        \prod_{i,j\geq1}(1-x_iy_j)^{-1} = \exp \log \prod_{i,j\geq1}(1-x_iy_j)^{-1} = \exp \sum_{n \geq 1} \frac{1}{n} p_n(\underline{X}) p_n(\underline{Y}).
    \]
    ...
    \begin{align*}
        ...
        t^{|\lambda|}
    \end{align*}
    Then, if we put $f(n) = p_n(\ulx)p_n(\uly)$,
    \begin{align*}
        \exp
        \sum_{n=1}^\infty
        \frac{t^n}{n}
        ...
        &=
        \sum_{\lambda \in \Par}
        p_{\lambda_1}(\ulx) p_{\lambda_1}(\uly)
        \cdots
        p_{\lambda_k}(\ulx) p_{\lambda_k}(\uly)
        z_\lambda^{-1}
        t^{|\lambda|} \\
        &=
        \sum_{\lambda \in \Par}
        \underbrace{
            \Big(
                p_{\lambda_1}(\ulx) \cdots p_{\lambda_k}(\ulx)
            \Big)
        }_{=p_\lambda(\ulx)}
        \underbrace{
            \Big(
                p_{\lambda_1}(\uly) \cdots p_{\lambda_k}(\uly)
            \Big)
        }_{=p_\lambda(\uly)}
        z_\lambda^{-1}
        t^{|\lambda|} \\
        &=
        \sum_{\lambda \in \Par}
        p_\lambda(\ulx) p_\lambda(\uly) z_\lambda^{-1}
        t^{|\lambda|}.
    \end{align*}
...
\begin{proof}
    \todo{Prove omega is an involution}
\end{proof}
...
\begin{definition} Let $\langle -,- \rangle$ be the scalar product defined by the relationship
    \[
        \langle m_\lambda, h_\nu \rangle \coloneq \delta_{\lambda\nu}.
    \]
    Where $\delta_{\lambda\nu} \coloneq [\lambda = \nu]^?$.

    This scalar product is called the \textit{Hall inner product}.
...
\begin{proof}[Proof (short)]
    ...
    \]
    Now, we will suppress the variables, so we will always take $u_\lambda$ to be $u_\lambda(\underline{X})$, and similarly we will always take $v_\lambda$ to mean $v_\lambda(\underline{Y})$.
    Similarly, we will take $m_\lambda$ to be in $\underline{X}$ and $h_\lambda$ to be in $\underline{Y}$.
...
\begin{definition}
    ...
    \[
        s_\lambda(\ulx)
        \coloneq
        \sum_{T\in\SSYT_{\shape \lambda}} \ulx^{\content T}.
    \]
\end{definition}
...
\begin{example}
    \[
        \ytabalign{bottom}
        \ulx^{\ytableaushort{11,23}} = x_1^2x_2x_3.
    \]
\end{example}
...
\begin{remark}
    ...
    \[
        s_\lambda = \sum_{\alpha\in\Com} K_{\lambda\alpha}x^\alpha.
    \]
    In particular,
    \[
        s_\lambda = \sum_{\mu\in\Par} K_{\lambda\mu}m_\mu.
    \]
\end{remark}
...
\begin{corollary}
    Fix $n$.
    $\{ s_\lambda \}_{\lambda \vdash n}$ forms a basis for $\Lambda^n$, as the transition matrix $(K_{\lambda\mu})_{n \preceq (\lambda,\mu) \preceq 1^n}$ is lower triangular.
...
\begin{theorem} Let $\lambda$ be a partition. Then
    \[
        s_\lambda = \det(h_{\lambda_i+j-i})_{1 \leq (i,j) \leq \len \lambda}.
    \]
\end{theorem}
...
\begin{definition}
    ...
    We define the polynomial $\alt_\gamma$ to be 
    \[
        \alt_\gamma = \det \left(x_j^{\gamma_i}\right)_{i,j=1}^n
    \]
\end{definition}

Put $\delta \coloneq (n-1,n-2,\ldots,0)$.
As a special case, we define the \textit{Vandermonde determinant} to be $\alt_\delta$.

\begin{definition}
    An \textit{alternating polynomial} is a polynomial $p$ such that
    \[
        \pi p = (\sgn \pi) \cdot p.
    \]
\end{definition}
...
By properties of the determinant, we have the following

\begin{remark}
    $\alt_\gamma$ is an alternating polynomial in $x_1,\ldots,x_n$ for all $\gamma \in \NN^n$.
\end{remark}

\begin{proposition} \label{prop:altpolyfactor}
    Any alternating polynomial $p(X_n)$ is divisible by $(x_i - x_j)$ for any $1 \leq i < j \leq n$.
\end{proposition}
...
\begin{theorem}
    ...
    \[
        \alt_\delta = \prod_{1 \leq i < j \leq n} (x_i-x_j).
    \]
\end{theorem}
...
\begin{proof}
    ...
    However, by Proposition \ref{prop:altpolyfactor}, we know that $\alt_\delta$ should factorize into \textit{at least} $n(n+1)/2$ factors!
...
\begin{theorem}
    RSK is a bijection $\Mat_\NN \leftrightarrow \biSSYT$ such that for all $A \rskarrow (P,Q)$,
    \begin{align*}
        \content P &= \col A, \\
        \content Q &= \row A.
    \end{align*}
...
\begin{proof}[Proof via RSK]
    \begin{align*}
        h_\mu = \sum_{\substack{A\in\Mat_\NN\\\row A=\mu}} x^{\col A}
              \qquad\rskarrow\qquad
              &=\left(\sum_{\substack{(P,Q)\in\biSSYT\\\ct P=\mu}} x^{\ct Q}\right)  \\
              &=\sum_{\lambda\in\Par}\left(\sum_{\substack{P,Q\in\SSYT_{\sh \lambda}\\\ct P=\mu}} x^{\ct Q}\right)  \\
              &=\sum_{\lambda\in\Par}\left(\sum_{P\in\SSYT_{\sh \lambda, \ct \mu}} \left(\sum_{Q\in\SSYT_{\sh \lambda}} x^{\ct Q}\right)\right) \\
              &=\sum_{\lambda\in\Par}\left(\substack{\#\text{of }\SSYT\text{ with}\\\text{shape }\lambda\text{ and content }\mu}\right)\left(\sum_{Q\in\SSYT_{\sh \lambda}} x^{\ct Q}\right) \\
              &= \sum_{\lambda\in\Par} K_{\sh\lambda,\ct\mu} \left(\sum_{Q\in\SSYT_{\sh \lambda}} x^{\ct Q}\right) \\
...
We have a special case of the forthcoming \textit{Littlewood-Richardson rule}, called the \textit{Pieri rule}.

\begin{definition}
    A \textit{horizontal strip} is a skew diagram such that each column contains at most one box.
\end{definition}
...
\begin{theorem}[Pieri rule]
    ...
    \]
    where $\mu$ ranges over all partitions obtained by adding a horizontal strip to $\lambda$.
\end{theorem}
...
The \textit{Murnaghan-Nakayama rule} is an analogous rule with the $p_\lambda$'s.
...
\begin{definition}
    Let $u = \underline{u_1 u_2 \cdots u_n}$ be a permutation.
    A \textit{elementary Knuth transformation} is a replacement of a substring of three consecutive letters given by the following rules
    \[
        \begin{cases}
            abc \equiv bac, \\
            cab \equiv cba, \\
...
\begin{definition}
    We say that two permutations $u$ and $v$ are \textit{Knuth-equivalent} whenever there is a sequence of \textit{elementary Knuth transformations} which turns one into another.
\end{definition}
...
\begin{proof}
    ...
    So, it suffices to prove it for a row.
    ...
    \begin{enumerate}
        ...
        \item 
            ...
            \begin{align*}
                ...
                \underset{
                    \hookleftarrow C_1
                }{\ytableaushort{a{C_2}b}}
                \xrightarrow{c}
                \underset{
                    \hookleftarrow C_1C_2
                }{\ytableaushort{acb}}
                \\
                &
                \ytableaushort{{C_1}{C_2}}
                \xrightarrow{b}
                \ytableaushort{{C_1}{C_2}b}
                \xrightarrow{a}
                \underset{
                    \hookleftarrow C_1
                }{\ytableaushort{a{C_2}b}}
