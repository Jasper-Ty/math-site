...
% Formal Power Series
\newcommand{\lrBrack}[1]{[\kern-1.2pt[ {#1} ]\kern-1.2pt]}
\newcommand{\fps}[2]{#1 \lrBrack{#2}}
...
\begin{remark}
    Consider the action $\KK\lrBrack{\bfx_\infty} \curvearrowleft S_\infty$ given by
    \[
        f(x_1,x_2,\ldots) \curvearrowleft w
        =
...
\begin{example}
    ...
    \[
        ...
        x_ix_j
    \]
    is a symmetric function that is homogeneous of degree $2$. In this case, 
    \[
        c_\alpha
        =
        \begin{cases}
            1,
            & \alpha = (\ldots,0,2,0,\ldots); \\
            10,
            & \alpha = (\ldots,0,1,0,\ldots,0,1,0,\ldots); \\
            0,
            & \text{otherwise}.
        \end{cases}
    \]
...
\subsection{The ring \texorpdfstring{$\Lambda$}{Λ} of symmetric functions}
...
\begin{proof}[Proof (sketch)]
    ...
    Since these two constructions specify the exact same subsets of $\NN^2$, they agree when taking unions and intersections, and so they both specify the same sublattice of $(\NN^2)$.
\end{proof}
...
The \defstyle{complete homogeneous symmetric functions}, or the \defstyle{completes}, have a very similar definition as the elementaries, but with distinctness relaxed.
...
\begin{definition}
    For all $\lambda \in \Par$ and $\alpha \in \Com$, we define $N_{\lambda\alpha}$ to be the coefficient of $x^\alpha$ in $h_\lambda$.
\end{definition}
...
\subsection{
    ...
    and \texorpdfstring{$\omega$}{ω}-involution
}
...
These can be used to define \defstyle{$\omega$-involution} on the ring of symmetric functions.

To define it, we note that $\Lambda = \pring{\QQ}{e_1,e_2,\ldots}$, which we recall means that $\Lambda$ is freely generated by the $e_n$'s as a commutative $\QQ$-algebra, any homomorphism $\phi$ out of $\Lambda$ can be defined by specifying the images of the $e_n$'s under $\phi$.
...
\begin{proof}
    ...
    Since $h_0 = e_0$, the statement clearly holds for $n=0$.
    Now, let $n > 0$, and suppose that $\omega(h_k) = e_k$ for all $k < n$.
    Then
    \begin{align*}
        \omega(h_n)
        &=
...
We define the following, natural, generating functions in the ring $\Lambda\lrBrack{t}$.
...
\begin{proof}
    ...
    \[
        ...
        \frac{1}{n} p_n(\bfx) p_n(\bfy).
    \]
    Then, we apply the \defstyle{permutation version of the exponential formula}, which tells us that, for any function $f: \PP \to \KK$ where $\KK$ is some commutative ring,
    \[
        \exp
        \left(
            \sum_{n=1}^\infty
...
\begin{theorem}
    [Knuth]
    Two words have the same insertion tableau if and only if they are Knuth equivalent.
\end{theorem}
...
\begin{corollary}
    Let $T$ be a semistandard tableau of shape $\lambda$, and let $\sfw = \rowword T$.
    Then
    \begin{enumerate}[label=(\alph*)]
        \item 
            $\lambda_1$ is the length of the longest non-decreasing subsequence of $\sfw$.
        \item 
            $\lambda^\top_1 = \ell(\lambda)$ is the length of the longest decreasing subsequence of $\sfw$.
    \end{enumerate}
\end{corollary}

\begin{proof}
    If $T$ has one row, this is clearly true.
\end{proof}

\begin{theorem}
    Knuth equivalence preserves the longest nondecreasing sequence and the longest decreasing subsequence.
\end{theorem}

\begin{corollary}
    Let $\sfw$ be a word, and let $\lambda = \shape P(\sfw)$.
    Then 
    \begin{enumerate}[label=(\alph*)]
        \item 
            $\lambda_1$ is the length of the longest non-decreasing subsequence of $\sfw$.
...
\begin{definition}
    ...
    An \defstyle{inner corner} of $\lambda / \mu$ is a box of $\mu$
\end{definition}
...
\begin{definition}
    A \defstyle{complete flag}
\end{definition}

\begin{definition}
    A \defstyle{Schubert cell} $\Omega^\circ_\lambda(F)$
\end{definition}
