...
\begin{definition}[$\glalg$, concretely]
    ...
    The \defstyle{general linear algebra} $\glalg_n(\FF)$ is defined
    \[
        \glalg_n(\FF)
        \coloneq
        \glalg\big(\Mat_n(\FF)\big).
    \]
\end{definition}
...
\begin{proposition}
    Let $\{e_{ij}\}_{i,j=0}^n$ be the standard basis of $\glalg_n(\FF)$.
    Then
    \[
        \lb{e_{ij}e_{kl}}
        =
...
\begin{definition}
    ...
    A \defstyle{bilinear form} is a function $\omega: V \times V \to \FF$ that is bilinear, i.e linear in each argument separately. 
\end{definition}
...
\begin{proposition}
    Let $\scrA$ be a $\FF$-algebra with product $\ast$.
...
\begin{proof}
    ...
    Then, $(\frkh + \FF g) \acts v = 0$, completing the theorem.
\end{proof}
...
\begin{proof}
    We will use Engel's theorem to demonstrate the nilpotence of the derived algebra $\lb{\frkg\frkg}$.
    \begin{itemize}
        \item[\textbf{Step 1}]
    \end{itemize}
...
\begin{definition}
    Let $\FF$ be a field.
    The \defstyle{$\mathbf{n}\times\mathbf{n}$ matrix ring} $\Mat_n(\FF)$ is defined to be the ring whose underlying set is $\FF^{n \times n}$ with pointwise scaling and addition, and with product given by matrix multiplication.
\end{definition}

\begin{definition}
    Let $V$ be a vector space over the field $\FF$.
    The \defstyle{dual space} $V^\ast$ of $V$ is the collection of all linear maps $V \to \FF$.
\end{definition}
...
\begin{definition}
    Let $V$ and $W$ be two $\FF$-vector spaces with bases $\bfv = (v_1,\ldots,v_n)$ and $\bfw = (w_1,\ldots,w_m)$ respectively.

    The \defstyle{tensor product of vector spaces} $V \otimes W$, is the $\FF$-vector space with basis
    \[
        \Big\{
...
\begin{definition}
    Let $V, W$ be $\FF$-vector spaces as before.
...
\begin{theorem}
    If $U$ and $V$ are two $\FF$-vector spaces, then any bilinear map $f: U \times V \to W$ factors through $\otimes: U \times V \to U \otimes V$--- there exists a unique linear map $\overline{f}$ that makes the following diagram commute:
    \[
        \begin{tikzcd}
...
\begin{definition}
    Let $U, V, X, Y$ be $\FF$-vector spaces, and let $f: U \to X$ and $g: V \to Y$ be linear maps.
...
\begin{definition}
    Let $V$ be a vector space over $\FF$ and fix a basis $\bfv = (v_1,\ldots,v_n)$ of $V$ with a dual basis $\bfv^\ast = (v^1, \ldots, v^n)$ of the dual space $V^\ast$.
    ...
    \[
        ...
        \sum_{i=1}^n 
        e^i \otimes v_i 
        \in (\FF^n)^\ast \otimes V
        ,\qquad
        \bfv^\ast 
        \coloneq 
        \sum_{i=1}^n v^i \otimes e_i 
        \in V^\ast \otimes \FF^n
    \]
...
\begin{theorem}
    Let $V$ be a vector space over $\FF$ of dimension $n$.
    Then
    \[
        \End V
        \simeq
...
\begin{proof}
    ...
    Then, the map $v^i \otimes v_j \mapsto e_{ij}$ provides the isomorphism between $V^\ast \otimes V$ and $M_n(\FF)$.
\end{proof}
...
\begin{proposition}
    ...
    \[
        \bfv\bfw^\ast \in (\FF^n)^\ast \otimes \FF^n
    \]
