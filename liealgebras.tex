...
\begin{theorem}[Schur's lemma]
    Let $V$ be an \textit{irreducible} $\frkg$-module.
    The only $\frkg$-module homomorphisms in $\End V$ are scalar multiples of the identity.
\end{theorem}
\begin{proof}
    Let $x \in \End V$ be a $\frkg$-module homomorphism.
    Since we are working over an algebraically closed field, we have at least one eigenvalue $\lambda$.
    
    Consider $x - \lambda I \in \gl(V)$.
    Since this is the sum of two $\frkg$-module homomorphisms, it is still a $\frkg$-module homomorphism.
    We have that $\ker (x - \lambda I) \neq 0$, as $(x - \lambda I) \acts v = 0$.
    Then $\ker (x - \lambda I) = V$, as a nonzero $\frkg$-submodule of the irreducible $\frkg$-module $V$.
    But this tells us that $x = \lambda I$, as desired.
\end{proof}


\begin{definition}[Tensor product of $\frkg$-modules]
    If $M$ and $N$ are two $\frkg$-modules,
\end{definition}
...
We describe all irreducible representations of $\frkg = \sl_2(\FF)$.
First, we fix the standard basis of $\sl_2$:
\[
    e 
    \defeq
    \begin{pmatrix}
    ...
    \end{pmatrix}.
\]
The bracket of $\sl_2$ is then determined by the relations
\[
    \lb{ef} = h, \quad
    \lb{he} = 2e, \quad
    \lb{hf} = -2f.
\]
Let $V$ be \textit{any} $\sl_2$-module.
Then the action of $h$ on $V$ is diagonal--- there exists a basis of $V$ for which the matrix representing the action of $h$ is a diagonal matrix.

This follows from the semisimplicity of $h$.

\begin{definition}
    For any $V$ be an $\sl_2$-module and $\lambda \in \FF$, we define the subspace $V_\lambda$ of $V$ by $V_\lambda = \{x \in V: h.x = \lambda x\}$.

    We call $V_\lambda$ a \defstyle{weight space}, corresponding to the \defstyle{weight} $\lambda$, if it is nonzero.
\end{definition}

Since, as noted, $h$ acts diagonally on $V$, it breaks up as a direct sum of weight spaces, that is, 
\[
    V
    =
    \bigoplus_\lambda
    V_\lambda
\]
where $\lambda$ runs over the weights $\lambda$ of $V$.

\begin{lemma}
    Let $V$ be an $\sl_2$-module and let $\lambda \in \FF$.
    If $x \in V_\lambda$, then $e.x \in V_{\lambda+2}$ and $f.x \in V_{\lambda-2}$.
\end{lemma}
\begin{proof}
    \begin{align*}
        h.e.x
...
Since $V$ is finite dimensional, there are only \emph{finitely} many such weight spaces.
There exists, then, a weight space $V_\lambda \neq 0$ for which $V_{\lambda+2} = 0$.
If $x \in V_\lambda$, it must be that $e.x = 0$.
We call $x$ a \defstyle{maximal vector}.

\begin{lemma}
    Let $V$ be an irreducible $\sl_2$-module, and let $x \in V_\lambda$ be a maximal vector.
    \begin{enumerate}
        [label=(\roman*)]
        \item
        \item 
